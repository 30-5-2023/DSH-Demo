...
本设计把 DSH Web 右侧的通用浏览页替换为业务用的工单产线看板，左侧对话结构保持不变。工单状态与执行独立于 DSH，由业务系统负责产线推进和事件发布，由右侧看板负责只读展示，由左侧 agent 通过 MCP 工具执行写操作。唤醒器只负责把业务系统判定为 `needsHuman` 的事件投递到对应会话，不把进度历史写入 agent 上下文。本文定义产品架构、责任边界、看板原型和三个关键过程；接口路径与 mock 的已实现限制由 [workorder-service/README.md](workorder-service/README.md) 维护。交互原则是“右边看，左边说”：产线自行运行，需要人时 AI 在左侧询问，用户回复后由 AI 执行操作。
...
| 唤醒器 | 计划实现的常驻接线小件：订产线事件流，把需要人的事件以用户消息的口吻投进对应会话（模拟人传话），并持久化会话绑定、消费游标和唤醒预算。今日还没有可复用的业务实现；换 agent 宿主只改它的投递目标 |
...
- **状态与三组对外接口**：产线状态、事件流 + 读接口 / 事件流 / 工具面。当前 mock 暂存的绑定表不是目标接口，Task 1 提取契约后删除。
...
**状态提交与事件发布必须保持同一顺序。** 生产服务先持久化状态、决策记录和事件游标，再发布事件；同一工单的写操作按顺序串行化，重复请求不能重复推进。消费者按 `rev` 检测缺口并重新读取权威快照，不能把 SSE 当成可独立重放的完整状态。
...
| **工单看板**（右栏页面，直连服务） | 从 Host 取得当前会话选中的 `orderId`，拉 ① 全量、订 ② 增量，把工单画成产线看板 | **给人的**：只读的产线全貌（没有操作按钮） | 不做第二个写入口、不复制 agent 会话状态；直连不等于可写——服务对它只开读与事件流 |
...
| **唤醒器**（接线件，宿主无关） | 订 ② 事件流；从工具调用记「会话 ↔ 工单」绑定；持久化绑定、消费游标和预算；`needsHuman` 时按预算把提示投进对应会话 | **给 agent 的**：唤醒提示<br>**给看板的**：当前会话所选 `orderId` | 不自己判 `needsHuman`、不渲染看板、不碰工单数据、不让业务服务保存第二张绑定表 |

**工单服务和唤醒器的职责，以及核心约束 1 至 3，会牵动多个组件**：接口或唤醒口径一变，看板、唤醒器、agent 至少其一要跟着改。页面布局与配色可以独立演进，但字段含义、状态文案和可操作提示仍须与服务状态保持一致。

### 三组接口：谁调哪一组

服务在一个端口上开三组入口：**① 读接口给看板**（拉工单全量与决策记录）、**② 事件流给看板与唤醒器**（产线一变就推一条，谁都不用轮询）、**③ 工具面给 agent**（查工单、推进、改绑定、重跑）。完整路径清单与事件载荷**以 [`workorder-service/README.md`](workorder-service/README.md) 为准**——这份不重抄，两处抄同一份清单迟早漂移。

两件容易读拧的事：

- **看板调的真实路径不带 `/api`**：直连需要服务放行看板来源的跨域。当前 mock 没有 CORS 响应头，跨源看板接入前必须补上；真实部署的允许来源与认证归业务方标准。
- **当前 mock 的 `/bindings` 不是目标接口**：会话与工单的绑定属于唤醒器，因为业务服务不认识 Agent 宿主。Task 1 在提取仍有效的契约后删除这组路径，不能保留两份权威记录。
...
7. **写操作必须校验状态并支持幂等重试**——推进工具只允许操作当前活动，并检查允许的来源状态；请求携带幂等键和可选的期望 `rev`，重复提交返回原结果，过期状态明确返回冲突。工具的“已受理”结果带操作标识，供事件与审计记录关联。
...
原型是单文件，双击就能开：[`prototype/workorder-pane.html`](prototype/workorder-pane.html)。文件里带一圈演示外壳（切宽度、配色、模拟推进的按钮），只为让你看到各种状态，不属于产品。页面长这样：
...
- **驱动提示条**常驻在作业区底部，随当前一步变四种形态（示例文案，以原型为准）：自动执行中「无需操作」／等待人工「对 agent 说『开始第 3 步』」，质检步换成『给出质检结论』／这一步失败「改完输入后说『重跑第 3 步』」／其余状态「产线推进中」。它只负责告诉人**现在该不该我出手、要说话的话说什么**。
- **卡片上没有业务按钮**。页面可以切换只读视图、重试加载、选择工单和打开交付件，但所有工单写操作都从左侧对话进入。
- **当前工单跟随会话绑定**：默认显示当前会话最近活动的工单；MVP 只显示这一张，后续只读选择器可以切换该会话已经绑定的其他工单。
- **数据怎么来的**：目标接入由页面直连服务，先拉 `GET /orders/:id` 和 `GET /orders/:id/decisions`，再订 `GET /events[?orderId=]`。当前 ② 事件是活动状态或输入绑定变化的通知，不是完整 patch；看板收到事件后按 `orderId` 重新拉取详情和决策记录，不能只修改事件中携带的字段。快照和 SSE 都带 `rev`，客户端必须发现订阅前后的版本差异时重新拉快照；生产服务还要提供断线重放或重同步，否则无法保证看板和唤醒器不漏事件。页面从活动 `outputs` 聚合交付件，不依赖额外的交付件接口。每个可打开的交付件必须有稳定的资源标识或 URL；当前 mock 只有文件名，原型中的打开动作仍是占位。当前原型仍使用内置 mock 数据，尚未接入真实服务；服务当前没有 CORS 响应头，直接从 `file://` 打开原型也不能作为已验证的在线接入方式。
...
sequenceDiagram
    ...
    A->>S: ③ MCP get_order
    S-->>A: 产线 5 步，前两步自动，第 3 步需人工
    A->>S: ③ MCP start_order
    S-->>A: 已受理，current=running<br/>（立即返回，不等执行回执）
    A-->>U: 已启动；前两步自动，第 3 步需要你时我会提醒
    ...
    S->>P: ② SSE 步骤1 完成
    S->>S: 步骤2 交给执行实现 → 完成，产出 授信分析报告.md
    S->>P: ② SSE 步骤2 完成
    S->>S: 步骤3 轮到且为手工 → 等待人工
    S->>P: ② SSE 步骤3 等待人工（卡片变琥珀）
    S->>W: ② SSE 同一条事件，needsHuman=true
    W->>A: 投递唤醒提示（工单号·第 3 步·等你复核）
    Note over A: agent 醒来：先调 get_order 核实，再开口
    A-->>U: 第 3 步等你复核，材料齐了
```
...
sequenceDiagram
    ...
    S->>P: ② SSE 步骤3 等待人工（看板早就显示了同样的状态）
    S->>W: ② SSE needsHuman=true，line="⏸ 步骤3 等待人工"
    W->>A: 投递唤醒提示（工单号·第几步·要做什么）
    Note over A: 宿主适配：空闲时开新一轮；正忙时排进下一步上下文（约束 4）
    A->>S: ③ MCP get_order 确认第 3 步
    S-->>A: 第 3 步输入：报告 ← 第 2 步 · 年报 ← 工单材料
    A-->>U: 第 3 步等你复核，材料齐了
    U->>A: 年报随单给过了，直接开始吧
    A->>S: ③ MCP start_activity
    S->>P: ② SSE 步骤3 执行中
    S-->>A: 已提交
    A-->>U: 已开始；线下复核完成后告诉我
    U->>A: 第 3 步已复核完成
    A->>S: ③ MCP finish_activity
    S->>P: ② SSE 步骤3 已完成
    S-->>A: 已完成
    A-->>U: 已记录完成，产线继续推进
```

**要点**：唤醒只发生在这一个时刻。看板早就显示了同样的状态，但**只有需要人时才开口**。图上画的是正常路径：按约束 4，投递 ≠ 处理，唤醒只把提示送进队列，不承诺模型一定接话。
...
2. **右栏默认跟随最近活动工单。** MVP 只显示当前会话最近活动的一张工单；后续只读选择器可以切换该会话已绑定的工单。
3. **工单从 `ready` 启动。** `start_order` 只允许 `ready → running`；相同幂等键返回原结果，其他非法状态返回明确冲突。
4. **人工活动显式完成。** `automation` 只决定是否自动启动；`manual` 类型通过 `start_activity` 进入 `running`，在线下工作完成并由用户报告后，通过 `finish_activity` 完成。
