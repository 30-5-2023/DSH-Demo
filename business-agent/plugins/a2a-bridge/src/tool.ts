...
import { A2ABridgeError, type CallA2AAgentInput } from './types.ts'
...
export function createCallA2AAgentTool(
  ...
  return defineTool({
    name: 'call_a2a_agent',
    description: 'Call an A2A agent by its Agent Card URL. Local files are resolved from the current Session workspace. Pass task_id from an input-required result to answer that Task; context_id continues its conversation. Outbound authentication is not supported.',
    parameters: {
      ...
      task_id: { type: 'string', description: 'Remote Task identifier returned with input-required; send the answer in message.' },
      ...
      schema: {
        ...
        properties: {
          ...
          interaction: {
            ...
            properties: {
              text: { type: 'string' },
              data: { type: 'array', items: { type: 'json' } },
            },
          },
          files: {
            type: 'array',
    ...
    async execute(args, exec) {
      ...
      const result = await client.call(input, exec.signal, exec.agent?.session.header.cwd)
      const { interaction, ...rest } = result
      return {
        ...rest,
        ...(interaction === undefined ? {} : {
          interaction: {
            ...(interaction.text === undefined ? {} : { text: interaction.text }),
            ...(interaction.data === undefined ? {} : { data: [...interaction.data] }),
          },
        }),
      }
    },
