...
Implementation stays under `business-agent/` as Cordis plugins, a Bundle, and a Profile patch. It does not modify `packages/`, `apps/`, or the agent loop. A deterministic keyless scenario completes one order through replayed model output, the official MCP Client, asynchronous execution, SSE refresh, one human wake, manual completion, and the final right-Sidebar state. Persistence, recovery, security, deliverable reads, and session-aware multi-order navigation remain separate increments.
...
- A keyless Session snapshot and built-Web scenario complete one order across the real DSH page, official MCP Client, service, wake adapter, and right Sidebar.
