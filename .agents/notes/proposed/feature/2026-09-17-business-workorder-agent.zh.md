...
工单服务独立于 DSH。服务拥有工单状态、状态迁移、决策记录、HTTP 快照、SSE 通知和 MCP 写工具。DSH Host 插件拥有会话与工单绑定、事件游标、唤醒预算和消息投递。DSH Client 插件注册只读右侧 Sidebar 标签页，只从 Host 取得选中的 `orderId`，并直接从服务读取业务快照和 SSE。
...
实现以 Cordis 插件、组合包和 profile patch 的形式保存在 `business-agent/` 下，不修改 `packages/`、`apps/` 或 agent loop。MVP 先验证一个确定性工单，从用户请求依次经过原生 MCP、异步执行、SSE 刷新、人工唤醒、人工活动完成，最后到达右侧 Sidebar 完成状态。持久化、恢复、安全、交付件读取和多工单导航分别在后续增量中实现。
...
- 工单服务不依赖 DSH 即可构建和运行。
- 业务 profile 无需修改 `packages/` 或 `apps/` 即可加载 Host 与 Client 插件。
- 原生 MCP 调用在顶层 `tool/call` 事件中公开 `orderId`。
- 只有新的阻塞事件可以唤醒绑定 agent 或向其注入消息，事件重放不会重复投递。
- 右侧 Sidebar 保持只读，并在收到 SSE 通知后刷新权威快照。
- 一个确定性的无密钥场景可以跨真实 DSH 页面、MCP 服务、唤醒适配器和右侧 Sidebar 完成一张工单。

## Risks

MVP 把服务和 Host 状态保存在内存中，因此在持久化增量完成前，进程重启会丢失进度或路由。浏览器直读还需要开发环境 CORS 放行，部署认证与来源限制将在后续完成。绑定依赖原生工具，因此在显式绑定机制实现前不支持仅使用 PTC 的展示模式。
